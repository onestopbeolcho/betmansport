...
def calculate_tax(combined_odds: float, gross_return: int, stake: int = 0) -> tuple[int, bool, str]:
    """
    배트맨 세금 계산
    ...
    if gross_return > 2_000_000:
        taxable_amount = gross_return - stake  # 적중금 - 투자금 = 기타소득금액
        tax = int(taxable_amount * 0.22)
        is_taxed = True
...
async def calculate_tax_optimization(req: TaxCalcRequest):
    ...
    original_tax, original_is_taxed, tax_reason = calculate_tax(original_odds, original_gross, stake)
    ...
    if len(items) > 1:
        for i in range(len(items)):
            ...
            new_tax, new_is_taxed, _ = calculate_tax(new_odds, new_gross, stake)
    ...
    if stake > 10000:
        ...
        split_tax, split_taxed, _ = calculate_tax(original_odds, split_gross, split_stake)
